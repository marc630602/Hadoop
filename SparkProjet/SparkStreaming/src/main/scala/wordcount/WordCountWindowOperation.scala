...
object WordCountWindowOperation {
  def main(args: Array[String]): Unit = {
    ...
    //5.统计个数(无状态转换)
    val windowDuration = new Duration(6000)
    val slideDuration = new Duration(3000)
    val countStreams: DStream[(String, Int)] = wordAndOneStreams.reduceByKeyAndWindow((x: Int, y:Int) => (x + y), windowDuration, slideDuration)
